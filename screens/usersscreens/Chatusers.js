...
import { getDatabase, ref, push, set, serverTimestamp,query, get,onValue } from "firebase/database";
...
const Chatusers = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    if (route.params && route.params.userData) {
      ...
      const phoneNumber = userData.userID;
  ...
  const sendMessage = async () => {
    ...
    if (messageText.trim() !== "") {
      ...
      const db = getDatabase();
      const userRef = ref(db, `rooms/${userId}`);
      ...
      set(newRoomRef, roomData)
        .then(() => {
          navigation.setParams({ hasUnreadMessages: true });
